...
class ThumbnailCache {
    ...
    private let cache: NSCache = {
        ...
        cache.name = "com.theMikeSwan.TMSDocumentBrowser.thumbnailcache.cache"
    ...
    private let workerQueue: NSOperationQueue = {
        ...
        workerQueue.name = "com.theMikeSwan.TMSDocumentBrowser.thumbnailcache.workerQueue"
    ...
    private func loadThumbnailInBackgroundForURL(URL: NSURL, documentIdentifier: Int, alreadyCached: Bool) {
        self.workerQueue.addOperationWithBlock {
            if let thumbnail = self.loadThumbnailFromDiskForURL(URL) {
                ...
                let scaledThumb = UIGraphicsGetImageFromCurrentImageContext()
                ...
                */
                self.cache.setObject(scaledThumb, forKey: documentIdentifier)
                
                NSOperationQueue.mainQueue().addOperationWithBlock {
                    self.cleanThumbnailDocumentIDs.insert(documentIdentifier)
                    
                    self.URLsNeedingReload.insert(URL)
                ...
                if !alreadyCached {
                    let image = self.loadIconForURL(URL)
...
//            print("Failed to load docID and will display placeholder image for \(URL)")
        ...
        let loadedThumbnail = existingImage ?? loadIconForURL(URL)
            ...
            */
            ...
            guard let dictionary = thumbnailDictionary as? [String: UIImage],
                image = dictionary[NSThumbnail1024x1024SizeKey] else {
                    throw DocumentBrowserError.ThumbnailLoadFailed
            }
    ...
    private func loadIconForURL(URL: NSURL) -> UIImage {
        let docController = UIDocumentInteractionController(URL: URL)
        return docController.icons.last!
    }
}
