...
class RecentModelObject: NSObject, NSFilePresenter, ModelObject {
    ...
    static let lastAccessKey = "lastAccessed"
    ...
    required init?(coder aDecoder: NSCoder) {
        do {
            ...
            lastAccessed = aDecoder.decodeObjectOfClass(NSString.self, forKey: RecentModelObject.lastAccessKey) as! String
    ...
    func encodeWithCoder(aCoder: NSCoder) {
        do {
            ...
            aCoder.encodeObject(lastAccessed, forKey: RecentModelObject.lastAccessKey)
    ...
    private func refreshNameAndSubtitle() throws {
        // Changed where displayName comes from to allow Finder on OS X to show the same name. Apple's original process is commented out below. Finder pulls the name it shows from the URL so documents could end up with a different name on each platform confusing te user.
